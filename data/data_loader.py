...
def load_csv(file_path: str, sample_size: int, output_file_path: str) -> pd.DataFrame:
    ...
    Perform stratified sampling based on the 'diabetes' column to address class imbalance.
    Sample size is fixed (e.g., 5000) and distributed equally across 'diabetes' classes.
    ...
    # Validate if the 'diabetes' column exists
    if 'diabetes' not in df.columns:
        raise ValueError("The dataset does not have a 'diabetes' column for stratification.")
    ...
    if (diabetes_counts[0] < sample_size_per_class) or (diabetes_counts[1] < sample_size_per_class):
    ...
    sampled_0 = df[df['diabetes'] == 0].sample(n=sample_size_per_class, random_state=42)
    sampled_1 = df[df['diabetes'] == 1].sample(n=sample_size_per_class, random_state=42)
